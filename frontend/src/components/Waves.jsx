...
class Noise {
  ...
  seed(seed) {
    ...
    for (let i = 0; i < 256; i += 1) {
      const value = i & 1 ? this.p[i] ^ (nextSeed & 255) : this.p[i] ^ ((nextSeed >> 8) & 255)
      this.perm[i] = value
  ...
  perlin2(x, y) {
    ...
    const localX = x - xIndex
    const localY = y - yIndex
    xIndex &= 255
    yIndex &= 255

    const n00 = this.gradP[xIndex + this.perm[yIndex]].dot2(localX, localY)
    const n01 = this.gradP[xIndex + this.perm[yIndex + 1]].dot2(localX, localY - 1)
    const n10 = this.gradP[xIndex + 1 + this.perm[yIndex]].dot2(localX - 1, localY)
    const n11 = this.gradP[xIndex + 1 + this.perm[yIndex + 1]].dot2(localX - 1, localY - 1)
...
export default function Waves({
  ...
  useEffect(() => {
    ...
    function setLines() {
      ...
      const totalLines = Math.ceil(outerWidth / currentXGap)
      const totalPoints = Math.ceil(outerHeight / currentYGap)
    ...
    function drawLines() {
      ...
      linesRef.current.forEach((points) => {
        ...
        points.forEach((point, index) => {
          ...
          const nextPoint = moved(point, !isLast)
          const afterPoint = moved(points[index + 1] || points[points.length - 1], !isLast)
          ctx.lineTo(nextPoint.x, nextPoint.y)
          if (isLast) ctx.moveTo(afterPoint.x, afterPoint.y)
        })
      })
      ctx.stroke()
    }
